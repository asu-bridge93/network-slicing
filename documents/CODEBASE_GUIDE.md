...
* [0. 研究初心者のための背景知識](#0-研究初心者のための背景知識)
  * [0.1 この研究が解いている問題を一言で言うと？](#01-この研究が解いている問題を一言で言うと)
  * [0.2 キーワード解説](#02-キーワード解説)
  * [0.3 「強化学習（RL）」をざっくり理解する](#03-強化学習rlをざっくり理解する)
  * [0.4 このリポジトリで使われているアルゴリズムの解説](#04-このリポジトリで使われているアルゴリズムの解説)
  * [0.5 シミュレーションが1ステップで何をしているか](#05-シミュレーションが1ステップで何をしているか)
* [1. このリポジトリが何をしているか](#1-このリポジトリが何をしているか)
* [2. ディレクトリ構成](#2-ディレクトリ構成)
* [3. 環境コア層の詳細](#3-環境コア層の詳細)
* [4. 実験構築層の詳細](#4-実験構築層の詳細)
* [5. アルゴリズム層の詳細](#5-アルゴリズム層の詳細)
* [6. 実験スクリプトの実行フロー](#6-実験スクリプトの実行フロー)
* [7. データフロー図](#7-データフロー図)
* [8. 主要パラメータ一覧](#8-主要パラメータ一覧)
* [9. 結果の保存形式](#9-結果の保存形式)
* [10. リソース逼迫時の一律削減ロジック（`adjust_action`）](#10-リソース逼迫時の一律削減ロジックadjust_action)
* [11. SLA計算ロジック（SLA Violation の判定・KPI）](#11-sla計算ロジックsla-violation-の判定kpi)
* [12. 結果可視化スクリプト（plot_*.py）の役割](#12-結果可視化スクリプトplot_pyの役割)
* [13. QRアルゴリズムのパラメータ「cost / nocost」について](#13-qrアルゴリズムのパラメータcost--nocostについて)
* [14. eMBB と mMTC スライスの定義・設計](#14-embb-と-mmtc-スライスの定義設計)
* [15. 参考文献](#15-参考文献)
...
## 11. SLA計算ロジック（SLA Violation の判定・KPI）

各スライスのSLA（Service Level Agreement：通信品質の保証条件）を守れたかどうかの判定（KPIの評価）は、L1レベルでの通信結果を集約する **`slice_ran.py`** 内の **`compute_reward(self)`** 関数で行われます。

### mMTC スライスのKPI：【平均遅延（Delay）】
mMTCスライスでは **「平均遅延時間」** がSLAの絶対的な基準となります。
*   **コード実装** (`SliceRANmMTC.compute_reward`):
    ```python
    SLA_fulfilled = self.info['delay']/self.slots_per_step < self.SLA['delay']
    ```
*   1ステップ（デフォルト100スロット）間でかかった全デバイスの平均遅延時間が、SLAの許容上限値（例: `300` ms）より**短い**かどうかで判定されます。上限を下回っていればSLA達成、少しでも上回って遅延してしまえば「SLA違反」としてグラフのカウントが上がります。

### eMBB スライスのKPI：【平均スループット（Throughput）】
eMBBスライスでは **「平均スループット（通信速度）」** がSLAの基準となっています。
*   **コード実装** (`SliceRANeMBB.compute_reward`):
    ```python
    cbr_th = self.info['cbr_th']/self.observation_time > self.SLA['cbr_th']
    ```
*   規定の観測時間（1ステップ）内に送信できた合計ビット数から計算されるスループット `cbr_th` が、SLAで要求された下限値（閾値）を**上回っている**かどうかで判定されます。下回って通信が詰まってしまうと「SLA違反」になります。
*   ※コード内では各ユーザのパケットキュー長（`cbr_queue`）等の関連指標も計算されていますが、最終的な達成フラグにはスループットの条件が直接的に使われています。

これらの判定結果が各ステップの終わりに集約され、SLA違反があった場合には「マイナスのペナルティ（負の報酬 `reward`）」として基地局（AIエージェント）の強化学習に使われる仕組みです。
...
| **`plot_slices.py`** | **スライス個別でのSLA達成度（ネットワークスライシングの本質である隔離性の検証）** を行う専用スクリプトです（独自修正版）。<br>`python plot_slices.py 0 KBRL_99` のように実行すると、全加算ではなく「eMBBスライス（Slice 0, 1）」と「mMTCスライス（Slice 2）」それぞれのSLA違反率の推移を個別の折れ線グラフに分解して描画します。各スライスが一様に満たされているか、特定のスライスだけが犠牲になっていないかを分析するのに特化しています。 | `./figures/` 内 |
...
### 結果データ（_cost / _nocost のディレクトリ）の生成手順

実際に異なるシナリオ（例：Scenario 1）においてこれらQR系の結果データを作成し、`plot_results.py` などのプロット用スクリプトに「nocost」として正しく読み込ませ比較グラフにするためには、以下の手順を踏む必要があります（元の開発者が実験時におそらく手動で行っていた手順です）。

1.  **対象シナリオの変更**: `experiments_qr.py` 内の `scenarios = [0]` を、実行したいシナリオ（例: `[1]`）に変更する。
2.  **パラメータの設定**: `experiments_qr.py` にある `QR_PARAMS` 辞書の中の `resource_cost_factor` の値を書き換える。
    *   **nocost を生成する場合**: `0.0` にする
    *   **cost を生成する場合**: `3.0` など（論文設定等）にする
3.  **実行**: `python experiments_qr.py` を実行し、30回のランが終わるまで待つ。
4.  **ディレクトリのリネーム（重要）**: デフォルトでは `results/scenario_1/QR_15_0_0_2_01_cost0/`などの名前で出力されますが、可視化スクリプト側がこれを正しく認識・紐付けできるように、手動でフォルダ名を `..._nocost` や `..._cost` にリネーム（変更）する。

---

## 14. eMBB と mMTC スライスの定義・設計

このシミュレータの最大の特徴である「2種類の異なる通信性質を持つスライスの共存」は、物理レイヤーまで踏み込んだ本格的なモデリングのもと、以下のスクリプト群で定義されています。

### どこで定義されているか？
1.  **`scenario_creator.py`**:
    *   両スライスのトラフィック生成ルール（デバイス数、パケット到着率、SLAの基準値など）の初期パラメータを定義します。
    *   各シナリオ（Scenario 0〜3）でのスライスの同居構成（例: eMBBが3つ、mMTCが2個、等）を組み立てる本丸ファイルです。
2.  **`slice_ran.py`（および `slice_ran_gbr.py`）**:
    *   スライスごとの「SLA違反判定（報酬計算）」や、上位レイヤーでの状態変数の更新ロジックを管理します。
3.  **`slice_l1.py`**:
    *   `SliceL1eMBB` クラスや `SliceL1mMTC` クラスとして、スライスごとの物理レイヤー（Layer 1）でのデータ送受信の振る舞いを別々にシミュレートします。先ほどmMTC特有のバグが出たのも、ここにスライス特性の違いが表れていたためです。

### どのように定義・シミュレートされているか（違い）

*   **eMBB (Enhanced Mobile Broadband / 高速大容量通信)**
    *   **トラフィック特性**: 動画ストリーミングのような大容量で連続的なデータ転送を想定し、CBR (Constant Bit Rate) 等のモデルで継続して大量のパケットが発生します。
    *   **物理レイヤー (L1)**: ユーザーごとに SINR（電波状況）と変調方式を計算し、「プロポーショナル・フェア（Proportional Fair）」などのスケジューラを使って、基地局から割り振られたPRB（サブキャリアリソース）の中で限界までスループットを引き出す複雑な計算を行います。
    *   **SLA違反の基準**: 主に**「スループット（通信速度）」**に重きを置きます。決められた一定の通信速度（閾値）を下回ってしまった場合に「SLA違反」とカウントします。
*   **mMTC (Massive Machine Type Communication / 超大量端末接続)**
    *   **トラフィック特性**: スマートメーター等の無数のIoTデバイスからの通信を想定し、1つのスライスに**`n_devices = 1000` 機**ものデバイスがぶらさがっています。各端末はごく小さなパケットをランダムかつ非常に稀に送信してきます。
    *   **物理レイヤー (L1)**: 大容量を捌くのではなく「微小データを複数回再送（repetition）して長期間かけて確実に届ける」という動作をシミュレートしています。そのため、eMBBのような複雑なスケジューラやSNR計算とは全く異なる専用のコードを持っています。
    *   **SLA違反の基準**: 何よりも**「遅延（Delay）」**を重視しています。例えば `delay: 300` (ms) をSLAとした場合、一定期間内にパケットの再送・受信完了が間に合わず、この猶予を超過してしまった場合に「SLA違反」とカウントされます。
